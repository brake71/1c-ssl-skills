...
> **Полный список** (18 методов, включая `ПроверитьСертификат`, `СвойстваПодписи`, `РезультатПроверкиПодписи`, `ДатаПодписания`, `ОткрытьНастройкиЭлектроннойПодписиИШифрования`, `ПроверитьДоверенность`, `ДоступнаЭлектроннаяПодпись`) — в [`bsp-esign-mcd-key-methods.md`](bsp-esign-mcd-key-methods.md).
