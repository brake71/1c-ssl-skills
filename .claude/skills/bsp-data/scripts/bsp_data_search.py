#!/usr/bin/env python3
"""BSP Data search: lookup methods/modules in 1C configuration export.

Covers modules: ОбменДанными*, ЭлектроннаяПодпись*, КонтактнаяИнформация*,
АдресныйКлассификатор*, КалендарныеГрафики*, ГрафикиРаботы*, Валюты*,
Банки*, ВнешниеКомпоненты*, and corresponding subsystem modules.
"""
...
def cmd_method(args):
    ...
    for mod_name, bsl_path in list_common_modules(src):
        ...
        if found:
            break
    if not found:
        print(f"Method '{target}' not found in data BSP modules.")
...
def cmd_module(args):
    ...
    for mod_name, bsl_path in list_common_modules(src):
        if mod_name.lower() == target.lower():
            ...
            return
    print(f"Module '{target}' not found in data BSP modules.")
...
def cmd_modules_by_subsystem(args):
    ...
    if modules:
        print(f"Data BSP modules in subsystem '{subsystem_name}':")
        for m in sorted(modules):
            print(f"  {m}")
    else:
        print(f"No data BSP modules found in subsystem '{subsystem_name}'.")
...
def cmd_detect(args):
    ...
    print(f"Data BSP modules: {bsp_count}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BSP Data module/method search")
    sub = parser.add_subparsers(dest="command")

    p_method = sub.add_parser("method", parents=[parent], help="Find module containing export method")
    p_method.add_argument("name", help="Method name (e.g. ВернутьАдрес)")

    p_module = sub.add_parser("module", parents=[parent], help="Show module path and stable API methods")
    p_module.add_argument("name", help="Module name (e.g. ОбменДаннымиСервер)")

    p_subsys = sub.add_parser("modules-by-subsystem", parents=[parent], help="List data modules in subsystem")
    p_subsys.add_argument("name", help="Subsystem name (e.g. ОбменДанными)")
