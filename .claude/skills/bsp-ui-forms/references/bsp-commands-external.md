...
| `ДополнительныеОтчетыИОбработкиКлиент.ОткрытьСписокДополнительныхОтчетовИОбработок` | `ОткрытьСписокДополнительныхОтчетовИОбработок()` | Клиент | Открыть форму списка справочника «Дополнительные отчёты и обработки» (для администратора). В исходном коде — `#Область СлужебныйПрограммныйИнтерфейс`; БСП не гарантирует обратную совместимость | `ДополнительныеОтчетыИОбработкиКлиент.ОткрытьСписокДополнительныхОтчетовИОбработок();` | ⚠️ служебный |
...
| `СозданиеНаОсновании.ПриОпределенииКомандПодключенныхКОбъекту` | `ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды)` | Сервер | Внутренняя реализация БСП для регистрации команд «Создать на основании» — находится в `#Область СлужебныйПрограммныйИнтерфейс` модуля `СозданиеНаОсновании`. Прикладной код **не вызывает** и **не переопределяет** этот метод в `СозданиеНаОсновании`; для прикладного переопределения используйте `ПодключаемыеКомандыПереопределяемый.ПриОпределенииКомандПодключенныхКОбъекту` | `// не вызывается напрямую из прикладного кода` | ⚠️ служебный |

> **Полный список** (23 метода, включая `ПараметрыРазмещения`, `ПараметрыВыполненияКоманды`, `ЗагрузитьНастройки`) — в [`bsp-commands-external-key-methods.md`](bsp-commands-external-key-methods.md).
