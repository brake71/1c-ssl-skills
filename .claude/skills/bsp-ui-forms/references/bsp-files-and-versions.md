...
- `#Область ПрограммныйИнтерфейс` — стабильный API, можно вызывать из прикладного кода: `ВключеноВерсионированиеОбъекта`, `ВключитьВерсионированиеОбъекта`, `ЗаписатьНастройкуВерсионированияПоОбъекту`, `ПриСозданииНаСервере`, `ЗначениеФлажкаХранитьИсторию`.
- `#Область СлужебныеПроцедурыИФункции` — ⚠️ служебный, обратная совместимость не гарантируется: `СведенияОВерсииОбъекта`, `ОтчетПоВерсииОбъекта`, `ЗаписатьВерсиюОбъекта` (часть из них исторически вызывалась из прикладного кода, но в БСП 3.1.11 перенесена в служебную область).
...
| `ВерсионированиеОбъектов.СведенияОВерсииОбъекта` | `СведенияОВерсииОбъекта(Знач Ссылка, Знач НомерВерсии)` | Сервер | Прочитать данные сохранённой версии (возвращает структуру с реквизитами объекта на момент версии). В исходном коде — `#Область СлужебныеПроцедурыИФункции`; БСП не гарантирует обратную совместимость | `Сведения = ВерсионированиеОбъектов.СведенияОВерсииОбъекта(Ссылка, 3);` | ⚠️ служебный |
| `ВерсионированиеОбъектов.ОтчетПоВерсииОбъекта` | `ОтчетПоВерсииОбъекта(СсылкаНаОбъект, Знач ВерсияОбъекта = Неопределено, ПользовательскийНомерВерсии = Неопределено)` | Сервер | Сформировать табличный документ с реквизитами указанной версии объекта (печатная форма отчёта по версии). В исходном коде — `#Область СлужебныеПроцедурыИФункции` | `ТабДок = ВерсионированиеОбъектов.ОтчетПоВерсииОбъекта(Ссылка, 3);` | ⚠️ служебный |
| `ВерсионированиеОбъектовКлиент.ПоказатьИсториюИзменений` | `ПоказатьИсториюИзменений(Ссылка, ФормаВладелец)` | Клиент | Открыть форму истории изменений объекта (из формы объекта). В исходном коде — `#Область СлужебныеПроцедурыИФункции` | `ВерсионированиеОбъектовКлиент.ПоказатьИсториюИзменений(Объект.Ссылка, ЭтаФорма);` | ⚠️ служебный |

> **Полный список** (20 методов, включая `КОбъектуМожноПрисоединятьФайлы`, `ДобавитьФайлСДиска`, `ЗаполнитьПрисоединенныеФайлыКОбъекту`, `ЗначениеФлажкаХранитьИсторию`, `ПометитьНаУдалениеВерсииФайла`, `ОткрытьОтчетПоВерсииОбъекта`) — в [`bsp-files-and-versions-key-methods.md`](bsp-files-and-versions-key-methods.md).
