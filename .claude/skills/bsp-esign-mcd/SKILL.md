...
|---|---|---|---|---|---|
| `ЭлектроннаяПодпись.ИспользоватьЭлектронныеПодписи` | `ИспользоватьЭлектронныеПодписи()` | Сервер | Проверка, что подсистема включена функциональной опцией | стабильный | `Если ЭлектроннаяПодпись.ИспользоватьЭлектронныеПодписи() Тогда …` |
| `ЭлектроннаяПодпись.ПодписиОбъекта` | `ПодписиОбъекта(Объект, ДополнительныеПараметры = Неопределено)` | Сервер | Получить массив установленных подписей (свойства, отпечатки, статусы) | стабильный | `МассивПодписей = ЭлектроннаяПодпись.ПодписиОбъекта(ДокументСсылка);` |
| `ЭлектроннаяПодпись.ДобавитьПодпись` | `ДобавитьПодпись(Объект, Знач СвойстваПодписи, ИдентификаторФормы = Неопределено, ВерсияОбъекта = Неопределено, ЗаписанныйОбъект = Неопределено)` | Сервер | Записать подпись к объекту и установить `ПодписанЭП = Истина` | стабильный | `ЭлектроннаяПодпись.ДобавитьПодпись(ДокументОбъект, СвойстваПодписи, УИДФормы, ВерсияОбъекта);` |
| `ЭлектроннаяПодпись.ПроверитьПодпись` | `ПроверитьПодпись(МенеджерКриптографии, ИсходныеДанные, Подпись, ОписаниеОшибки = Null, НаДату = Неопределено, РезультатСтруктура = Неопределено)` | Сервер | Математическая проверка подписи + сертификата; возвращает `Истина/Ложь`, `ОписаниеОшибки` заполняется при неуспехе | стабильный | `Верна = ЭлектроннаяПодпись.ПроверитьПодпись(Менеджер, Данные, ДвоичныеДанныеПодписи, ОписаниеОшибки);` |
| `ЭлектроннаяПодпись.ПроверитьСертификат` | `ПроверитьСертификат(МенеджерКриптографии, Сертификат, ОписаниеОшибки = Null, НаДату = Неопределено, ПараметрыПроверки = Неопределено)` | Сервер | Проверка только сертификата (без подписи) | стабильный | `Верн = ЭлектроннаяПодпись.ПроверитьСертификат(Менеджер, Сертификат, Ошибка);` |
| `ЭлектроннаяПодпись.СвойстваПодписи` | `СвойстваПодписи(Подпись, ПрочитатьСертификаты = Истина)` | Сервер | Извлечь свойства подписи (отпечаток, кем выдана, дата) | стабильный | `Св = ЭлектроннаяПодпись.СвойстваПодписи(ДвоичныеДанныеПодписи);` |
| `ЭлектроннаяПодпись.МенеджерКриптографии` | `МенеджерКриптографии(Операция, ПоказатьОшибку = Истина, ОписаниеОшибки = "", Программа = Неопределено)` | Сервер | Получить `МенеджерКриптографии` с настройками из ИБ | стабильный | `Менеджер = ЭлектроннаяПодпись.МенеджерКриптографии("Подписание", Ложь, Ошибка);` |
| `ЭлектроннаяПодписьКлиент.Подписать` | `Подписать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено, ПараметрыПодписи = Неопределено)` | Клиент | Асинхронное подписание: открывает форму выбора сертификата, результат — в `ОбработкаРезультата` или `ОписаниеДанных.СвойстваПодписи` | стабильный | `ЭлектроннаяПодписьКлиент.Подписать(ОписаниеДанных, ЭтаФорма, ОповещениеОЗавершении);` |
| `ЭлектроннаяПодписьКлиент.ПроверитьПодпись` | `ПроверитьПодпись(Оповещение, ИсходныеДанные, Подпись, МенеджерКриптографии = Неопределено, НаДату = Неопределено, ПараметрыПроверки = Неопределено)` | Клиент | Асинхронная проверка; результат в `Оповещение` (Булево/Строка/структура) | стабильный | `ЭлектроннаяПодписьКлиент.ПроверитьПодпись(Оповещение, Данные, Подпись, Неопределено, Дата, ПараметрыПроверки);` |
| `ЭлектроннаяПодписьКлиент.ДатаПодписания` | `ДатаПодписания(Оповещение, Подпись, ПривестиКЧасовомуПоясуСеанса = Истина)` | Клиент | Извлечь дату из данных подписи; результат — `Дата` или `Неопределено` в оповещении | стабильный | `ЭлектроннаяПодписьКлиент.ДатаПодписания(Оповещение, ДвоичныеДанныеПодписи);` |
| `ЭлектроннаяПодписьКлиент.ДобавитьСертификат` | `ДобавитьСертификат(ОбработчикЗавершения = Неопределено, ПараметрыДобавления = Неопределено)` | Клиент | Открыть форму добавления сертификата в справочник; результат — в оповещении | стабильный | `ЭлектроннаяПодписьКлиент.ДобавитьСертификат(ОповещениеОЗавершении, ПараметрыДобавления);` |
| `ЭлектроннаяПодписьКлиент.ПолучитьОтпечаткиСертификатов` | `ПолучитьОтпечаткиСертификатов(Оповещение, ТолькоЛичные, ПараметрыПолучения = Истина)` | Клиент | Список отпечатков доступных сертификатов (личные/все); результат — массив строк в оповещении | стабильный | `ЭлектроннаяПодписьКлиент.ПолучитьОтпечаткиСертификатов(Оп, Истина);` |
| `ЭлектроннаяПодписьКлиент.ОткрытьНастройкиЭлектроннойПодписиИШифрования` | `ОткрытьНастройкиЭлектроннойПодписиИШифрования(Страница = "Сертификаты")` | Клиент | Открыть общую форму настроек подсистемы | стабильный | `ЭлектроннаяПодписьКлиент.ОткрытьНастройкиЭлектроннойПодписиИШифрования();` |
| `ЭлектроннаяПодписьКлиентСервер.НовыеСвойстваПодписи` | `НовыеСвойстваПодписи()` | Клиент + Сервер | Конструктор структуры свойств подписи (отпечаток, дата, сертификат, статус) | стабильный | `Св = ЭлектроннаяПодписьКлиентСервер.НовыеСвойстваПодписи();` |
| `ЭлектроннаяПодписьКлиентСервер.РезультатПроверкиПодписи` | `РезультатПроверкиПодписи()` | Клиент + Сервер | Конструктор структуры результата проверки (Используется в `ПроверитьПодпись` с `ПараметрыПроверки.РезультатВВидеСтруктуры = Истина`) | стабильный | `Рез = ЭлектроннаяПодписьКлиентСервер.РезультатПроверкиПодписи();` |
| `МашиночитаемыеДоверенностиФНС.ДобавитьПодписьКФайлуДоверенности` | `ДобавитьПодписьКФайлуДоверенности(ФайлДоверенности, Знач Подпись)` | Сервер | Добавить подпись к присоединённому файлу МЧД; возвращает `Истина` или текст ошибки | стабильный | `Рез = МашиночитаемыеДоверенностиФНС.ДобавитьПодписьКФайлуДоверенности(ФайлМЧД, СвойстваПодписи);` |
| `МашиночитаемыеДоверенностиФНС.РезультатПроверкиПодписиПоМЧД` | `РезультатПроверкиПодписиПоМЧД(ПодписанныйОбъект, ИдентификаторПодписи, СертификатПодписи, НаДату)` | Сервер | Проверить записанную подпись на соответствие МЧД (реестр ФНС, полномочия, дата) | стабильный | `М = МашиночитаемыеДоверенностиФНС.РезультатПроверкиПодписиПоМЧД(Док, ИдПодп, Серт, Дата);` |
| `МашиночитаемыеДоверенностиФНСКлиент.ОткрытьСписокМЧД` | `ОткрытьСписокМЧД(Отборы = Неопределено, ОповещениеОЗакрытии = Неопределено, Владелец = Неопределено)` | Клиент | Открыть форму списка МЧД с произвольным отбором | стабильный | `МашиночитаемыеДоверенностиФНСКлиент.ОткрытьСписокМЧД(Отбор, Оповещение);` |
| `МашиночитаемыеДоверенностиФНСКлиент.СоздатьМЧД` | `СоздатьМЧД(ПараметрыФормы, ОповещениеОЗавершении = Неопределено)` | Клиент | Открыть форму создания новой МЧД | стабильный | `МашиночитаемыеДоверенностиФНСКлиент.СоздатьМЧД(Параметры, Оповещение);` |
| `МашиночитаемыеДоверенностиФНСКлиент.ПроверитьДоверенность` | `ПроверитьДоверенность(Оповещение, Доверенность, ИдентификаторФормы = Неопределено)` | Клиент | Асинхронная проверка доверенности в реестре ФНС | стабильный | `МашиночитаемыеДоверенностиФНСКлиент.ПроверитьДоверенность(Оп, МЧДСсылка);` |
| `ЭлектроннаяПодписьСлужебный.Зашифровать` | `Зашифровать(Данные, Сертификат, МенеджерКриптографии)` | Сервер | Серверное шифрование для массива получателей | ⚠️ служебный | `Данные = ЭлектроннаяПодписьСлужебный.Зашифровать(ДвоичныеДанные, МассивСертификатов, Менеджер);` |
| `ЭлектроннаяПодписьСлужебный.ДоступнаЭлектроннаяПодпись` | `ДоступнаЭлектроннаяПодпись(ТипОбъекта)` | Сервер | Проверить, что у типа объекта есть реквизит `ПодписанЭП` (через `ОпределяемыйТип.ПодписанныйОбъект`) | ⚠️ служебный | `Если ЭлектроннаяПодписьСлужебный.ДоступнаЭлектроннаяПодпись(ТипЗнч(Док)) Тогда …` |
