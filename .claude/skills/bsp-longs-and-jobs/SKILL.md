...
| Метод | Сигнатура | Сервер/Клиент | Назначение | Стабильность |
|---|---|---|---|---|
| `ДлительныеОперации.ВыполнитьФункцию` | `ВыполнитьФункцию(Знач ПараметрыВыполнения, ИмяФункции, Знач Параметр1 = Неопределено, ..., Знач Параметр7 = Неопределено)` | Сервер, ВнешнееСоединение | Запуск функции в фоне; возвращает `Структура` со `Статус`, `ИдентификаторЗадания`, `АдресРезультата` | ✅ **предпочтительный** |
| `ДлительныеОперации.ВыполнитьПроцедуру` | `ВыполнитьПроцедуру(Знач ПараметрыВыполнения = Неопределено, ИмяПроцедуры, Знач Параметр1 = Неопределено, ..., Знач Параметр7 = Неопределено)` | Сервер, ВнешнееСоединение | Запуск процедуры в фоне; без возвращаемого значения | ✅ **предпочтительный** |
| `ДлительныеОперации.ВыполнитьВФоне` | `ВыполнитьВФоне(Знач ИмяПроцедуры, Знач ПараметрыПроцедуры, Знач ПараметрыВыполнения)` | Сервер, ВнешнееСоединение | Запуск через структуру параметров; рекомендуется заменить на `ВыполнитьФункцию` / `ВыполнитьПроцедуру` | ⚠️ **deprecated** (документация рекомендует `ВыполнитьФункцию`/`ВыполнитьПроцедуру`) |
| `ДлительныеОперации.ПараметрыВыполненияФункции` | `ПараметрыВыполненияФункции(Знач ИдентификаторФормы)` | Сервер | Конструктор структуры параметров для `ВыполнитьФункцию` (аналог `ПараметрыВыполненияПроцедуры()` — без параметров) | ✅ стабильный |
| `ДлительныеОперации.СообщитьПрогресс` | `СообщитьПрогресс(Знач Процент = Неопределено, Знач Текст = Неопределено, Знач ДополнительныеПараметры = Неопределено)` | Сервер (в фоне) | Сообщить процент/текст; вызывать в теле фоновой процедуры | ✅ стабильный |
| `ДлительныеОперации.ОтменитьВыполнениеЗадания` | `ОтменитьВыполнениеЗадания(Знач ИдентификаторЗадания)` | Сервер | Отменить фоновое задание по идентификатору | ✅ стабильный |
| `ДлительныеОперацииКлиент.ОжидатьЗавершение` | `ОжидатьЗавершение(Знач ДлительнаяОперация, Знач ОповещениеОЗавершении = Неопределено, Знач ПараметрыОжидания = Неопределено)` | Клиент | Подключить обработчик оповещения и открыть форму ожидания | ✅ стабильный |
| `ДлительныеОперацииКлиент.ПараметрыОжидания` | `ПараметрыОжидания(ФормаВладелец)` | Клиент | Конструктор параметров ожидания: `ВыводитьОкноОжидания`, `ВыводитьПрогрессВыполнения`, `ОповещениеОПрогрессеВыполнения`, `ВыводитьСообщения`, `Интервал` | ✅ стабильный |
| `РегламентныеЗаданияСервер.НайтиЗадания` | `НайтиЗадания(Отбор)` | Сервер, ВнешнееСоединение | Найти регламентные задания по отбору (`Метаданные`, `Ключ`, `Использование`, `УникальныйИдентификатор`) | ✅ стабильный |
| `РегламентныеЗаданияСервер.Задание` | `Задание(Знач Идентификатор)` | Сервер, ВнешнееСоединение | Получить одно задание (по `УникальныйИдентификатор`, имени метаданных или объекту метаданных) | ✅ стабильный |
| `РегламентныеЗаданияСервер.ДобавитьЗадание` | `ДобавитьЗадание(Параметры)` | Сервер, ВнешнееСоединение | Создать задание; `Параметры` — структура (`Метаданные`, `Расписание`, `Ключ`, `Параметры`, `Использование`) | ✅ стабильный |
| `РегламентныеЗаданияСервер.ИзменитьЗадание` | `ИзменитьЗадание(Знач Идентификатор, Знач Параметры)` | Сервер, ВнешнееСоединение | Изменить свойства задания | ✅ стабильный |
| `РегламентныеЗаданияСервер.УдалитьЗадание` | `УдалитьЗадание(Знач Идентификатор)` | Сервер, ВнешнееСоединение | Удалить задание | ✅ стабильный |
| `РегламентныеЗаданияСервер.УстановитьРасписаниеРегламентногоЗадания` | `УстановитьРасписаниеРегламентногоЗадания(Знач Идентификатор, Знач Расписание)` | Сервер, ВнешнееСоединение | Установить `РасписаниеРегламентногоЗадания` | ✅ стабильный |
| `РегламентныеЗаданияСервер.УстановитьИспользованиеРегламентногоЗадания` | `УстановитьИспользованиеРегламентногоЗадания(Знач Идентификатор, Знач Использование)` | Сервер, ВнешнееСоединение | Включить/выключить задание | ✅ стабильный |
| `РегламентныеЗадания.РасписаниеВСтруктуру` | `РасписаниеВСтруктуру(Знач Расписание)` | Сервер, ВнешнееСоединение | `РасписаниеРегламентногоЗадания` → `Структура` (для передачи на клиент) | ✅ стабильный |
| `РегламентныеЗадания.СтруктураВРасписание` | `СтруктураВРасписание(Знач СтруктураРасписания)` | Сервер, ВнешнееСоединение | `Структура` → `РасписаниеРегламентногоЗадания` | ✅ стабильный |
| `РегламентныеЗаданияСлужебный.ВыполнитьРегламентноеЗаданиеВручную` | `ВыполнитьРегламентноеЗаданиеВручную(Знач Задание)` | Сервер, ВнешнееСоединение | Запустить задание немедленно (вручную), а не по расписанию | ⚠️ служебный |
| `РегламентныеЗаданияСлужебный.ПолучитьСвойстваФоновогоЗадания` | `ПолучитьСвойстваФоновогоЗадания(Идентификатор, ИменаСвойств = "")` | Сервер, ВнешнееСоединение | Свойства конкретного фонового задания по `ИменаСвойств` | ⚠️ служебный |
| `РегламентныеЗаданияПереопределяемый.ПриОпределенииНастроекРегламентныхЗаданий` | `ПриОпределенииНастроекРегламентныхЗаданий(Настройки)` | Сервер, ВнешнееСоединение | **Переопределение** (не вызов): настройка блокировки регл. заданий в модели сервиса | ✅ стабильный (для переопределения) |
...
### Где искать модули в дереве конфигурации
...
- `ДлительныеОперацииКлиент` — `CommonModules/ДлительныеОперацииКлиент/Ext/Module.bsl` (Клиент).
- `ДлительныеОперацииВызовСервера` — `CommonModules/ДлительныеОперацииВызовСервера/Ext/Module.bsl` (Сервер; только `ФоновоеЗаданиеЗавершено`).
- `РегламентныеЗаданияСервер` — `CommonModules/РегламентныеЗаданияСервер/Ext/Module.bsl` (Сервер, ВнешнееСоединение).
- `РегламентныеЗаданияКлиент` — `CommonModules/РегламентныеЗаданияКлиент/Ext/Module.bsl` (Клиент).
- `РегламентныеЗаданияСлужебный` — `CommonModules/РегламентныеЗаданияСлужебный/Ext/Module.bsl` (Сервер; ⚠️ служебный API).
- `РегламентныеЗаданияПереопределяемый` — `CommonModules/РегламентныеЗаданияПереопределяемый/Ext/Module.bsl` (Сервер; **переопределение**, не вызов).
- `РегламентныеЗадания` (без суффикса) — `CommonModules/РегламентныеЗадания/Ext/Module.bsl` (Сервер; **только** конвертация расписания).

Точка входа в подсистемы:
- `Subsystems/СтандартныеПодсистемы/Subsystems/БазоваяФункциональность.xml` — содержит `ДлительныеОперации`, `ДлительныеОперацииКлиент`, `ДлительныеОперацииВызовСервера` и др.
- `Subsystems/СтандартныеПодсистемы/Subsystems/РегламентныеЗадания.xml` — отдельная подсистема.

### Grep-шаблоны

```text
# Все экспортные методы в `ДлительныеОперации` (для оценки размера API)
^(Функция|Процедура) [А-Я][А-Яа-яA-Za-z_]+\(.*\) Экспорт   — в модуле `ДлительныеОперации`

# Граница устаревшей области в `ДлительныеОперации`
^#Область УстаревшиеПроцедурыИФункции   — в модуле `ДлительныеОперации`

# Стабильный API `РегламентныеЗаданияСервер`
^#Область ПрограммныйИнтерфейс   — в модуле `РегламентныеЗаданияСервер`

# Служебный API `РегламентныеЗаданияСлужебный` (⚠️ — обратная совместимость не гарантируется)
^#Область СлужебныйПрограммныйИнтерфейс   — в модуле `РегламентныеЗаданияСлужебный`

# Все вызовы `ДлительныеОперации.ВыполнитьВФоне` в прикладном коде (для миграции на `ВыполнитьФункцию`/`ВыполнитьПроцедуру`)
ДлительныеОперации\.ВыполнитьВФоне   — по всем общим модулям и прикладным объектам
```

### Glob-маски

- `CommonModules/ДлительныеОперации*/Ext/Module.bsl` — все варианты контекста (`ДлительныеОперации`, `ДлительныеОперацииКлиент`, `ДлительныеОперацииВызовСервера`).
- `CommonModules/РегламентныеЗадания*/Ext/Module.bsl` — все 5 модулей семейства (`РегламентныеЗадания`, `…Клиент`, `…Сервер`, `…Переопределяемый`, `…Служебный`).
- `Subsystems/СтандартныеПодсистемы/Subsystems/БазоваяФункциональность.xml` — точка входа для `ДлительныеОперации` (входит как функциональный блок).
- `Subsystems/СтандартныеПодсистемы/Subsystems/РегламентныеЗадания.xml` — точка входа для подсистемы.

### На что обратить внимание в дереве метаданных

- **`ПодпискиНаСобытия`** в `CommonModules/ДлительныеОперации` — `УстановкаПараметровСеанса`, `ПриДобавленииСерверныхОповещений`, `ПриОтправкеСерверногоОповещения` (служебный API, инициализация интеграции).
- **`ОпределяемыеТипы`** — `ДлительнаяОперация` (тип структуры-результата `ВыполнитьФункцию`/`ВыполнитьПроцедуру`).
- **`РегламентныеЗадания`** как **объект метаданных конфигурации** — это не модуль, а набор предопределённых заданий (`Метаданные.РегламентныеЗадания.Найти(...)`). Используется в `ДобавитьЗадание(Параметры).Метаданные` для указания, на основе какого шаблона создавать задание.
- В **формах** типичный пользователь `ДлительныеОперацииКлиент.ОжидатьЗавершение` — обработчик `КомандаФормы` с `ОписаниеОповещения` в `ЭтотОбъект`.
- В `РегламентныеЗаданияСервер.ДобавитьЗадание(Параметры)` **обязательно** свойство `Метаданные` — `ОбъектМетаданныхРегламентноеЗадание` или его `Имя`. Без него `ДобавитьЗадание` вернёт ошибку.
