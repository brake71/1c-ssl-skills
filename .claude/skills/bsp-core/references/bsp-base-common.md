...
Все модули `ОбщегоНазначения*`, `СтроковыеФункции*`, `ФайловаяСистема*` расположены в `CommonModules/` (BSL/XML). В выгрузке конфигурации исходники лежат в `CommonModules/<ИмяМодуля>/Ext/Module.bsl`. Агент **сам читает реальный код модуля**, а не абстрактную документацию.
